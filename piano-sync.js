(()=>{
'use strict';
if(window.__circulosPianoSyncV6)return;
window.__circulosPianoSyncV6=true;
...
const activePointers=new Map(),blockedClicks=new WeakMap(),visualTimers=new WeakMap(),rangeBindings=new WeakMap(),initialPositionDone=new WeakSet();
...
#performancePianoKeyboard .white-key.voicing-tone{background:var(--root)}
#performancePianoKeyboard .white-key.root-tone{background:var(--root2);color:#fff}
...
#performancePianoKeyboard .black-key.voicing-tone{background:var(--root)}
#performancePianoKeyboard .black-key.root-tone{background:var(--root2)}
...
function updateRange(binding){const max=Math.max(0,binding.scroll.scrollWidth-binding.scroll.clientWidth);binding.range.disabled=max<2;if(!binding.dragging)binding.range.value=max?String(Math.round(binding.scroll.scrollLeft/max*1000)):'0';}
function setInitialRightPosition(binding){
  if(initialPositionDone.has(binding.host))return;
  const max=Math.max(0,binding.scroll.scrollWidth-binding.scroll.clientWidth);if(max<2)return;
  binding.scroll.scrollLeft=max;binding.range.value='1000';initialPositionDone.add(binding.host);
}
function bindRange(host,control){
  ...
  range.addEventListener('pointerdown',()=>{binding.dragging=true;});range.addEventListener('pointerup',()=>{binding.dragging=false;updateRange(binding);});range.addEventListener('pointercancel',()=>{binding.dragging=false;updateRange(binding);});
  range.addEventListener('input',()=>{const max=Math.max(0,scroll.scrollWidth-scroll.clientWidth);scroll.scrollLeft=max*(Number(range.value)/1000);});
  scroll.addEventListener('scroll',()=>updateRange(binding),{passive:true});
  if(window.ResizeObserver)new ResizeObserver(()=>{updateRange(binding);requestAnimationFrame(()=>setInitialRightPosition(binding));}).observe(scroll);
  new MutationObserver(()=>requestAnimationFrame(()=>{updateRange(binding);setInitialRightPosition(binding);})).observe(host,{childList:true,subtree:true});
  requestAnimationFrame(()=>{updateRange(binding);requestAnimationFrame(()=>setInitialRightPosition(binding));});
}
